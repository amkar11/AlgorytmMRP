import pandas as pd
ilość_artykuły = int(input("Proszę podać ilość artykuły "))
for i in range(ilość_artykuły):
    nazwa_produktu = input("Proszę wskazać nazwę produktu ")
    poziom_produktu = input("Proszę wskazać poziom produktu ")
    ilość_wierszy = int(input("Ile tygodni trzeba na wykonanie zamówienia? "))
    columns = {"Tydzień":[i for i in range(1, ilość_wierszy+1)], "Potrzeby brutto":0, "Wstępny zapas":0, "Potrzeby netto":0, "Wstępne zmontowanie":0, "Zaplanowany odbiór":0}
    row_labels = [i for i in range(1, ilość_wierszy+1)]
    data = pd.DataFrame(data = columns, index=row_labels)
    def potrzeby_brutto():
        ilość_dostarczeń = int(input("Proszę wskazać ilość dostarczeń "))
        for i in range(ilość_dostarczeń):
            dzień_dostarczania = int(input("Proszę wskazać tydzień dostarczania produktu "))
            ilość_produktów = int(input("Proszę wskazać ilość produktów "))
            data.loc[dzień_dostarczania, "Potrzeby brutto"] = ilość_produktów
    potrzeby_brutto()
    def wstepny_zapas():
